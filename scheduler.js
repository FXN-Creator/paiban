...
function performScheduling() {
    try {
        // 显示加载状态
        const loadingElement = createLoadingElement('正在生成排班表...');
        ...
        if (isNaN(count) || count < 1) {
            showErrorMessage('请输入有效的人数');
            document.body.removeChild(loadingElement);
            return;
        }
        
        if (count > schedulingRules.maxPeople) {
            showErrorMessage(`人数不能超过${schedulingRules.maxPeople}人`);
            document.body.removeChild(loadingElement);
            return;
        }
        ...
        for (let i = 1; i <= count; i++) {
            ...
            if (restDays.length !== schedulingRules.restDaysPerPerson) {
                showErrorMessage(`人员 ${i} 必须设置${schedulingRules.restDaysPerPerson}个休息日`);
                document.body.removeChild(loadingElement);
                return;
            }
            ...
            if (uniqueRestDays.length !== restDays.length) {
                showErrorMessage(`人员 ${i} 的休息日不能重复`);
                document.body.removeChild(loadingElement);
                return;
            }
            
            people[i-1] = { name, restDays, stats: { morning: 0, evening: 0, rest: schedulingRules.restDaysPerPerson, total: 0 } };
        }
        ...
        if (huangJinyu) {
            ...
            if (invalidDays.length > 0) {
                showErrorMessage(`黄锦钰的休息日 ${invalidDays.join(', ')} 超出了${month}月的范围（1-${daysInMonth}），请修改后重新提交`);
                document.body.removeChild(loadingElement);
                return;
            }
        }
        ...
        // 检查多人在同一天休息的情况
        const restDayCounts = new Array(daysInMonth + 1).fill(0);
        ...
        // 显示成功消息
        showSuccessMessage('排班表生成成功！');
    } catch (error) {
        console.error('排班失败:', error);
        showErrorMessage('排班过程中发生错误，请重试。');
...
function displayResult(schedule, year, month, people, department) {
    ...
    h2.textContent = `${year}年${month}月排班表`;
    ...
    deptTh.textContent = '部门';
    headerRow.appendChild(deptTh);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayTh.innerHTML = `${day}<br>${weekDay}`;
        headerRow.appendChild(dayTh);
    }
    
    // 统计列
    const morningTh = document.createElement('th');
    morningTh.textContent = '早班';
    headerRow.appendChild(morningTh);
    
    if (schedulingRules.shiftTypes === 3) {
        const midTh = document.createElement('th');
        midTh.textContent = '中班';
        headerRow.appendChild(midTh);
    }
    
    const eveningTh = document.createElement('th');
    eveningTh.textContent = '晚班';
    headerRow.appendChild(eveningTh);
    
    const restTh = document.createElement('th');
    restTh.textContent = '休息';
    headerRow.appendChild(restTh);
    ...
    people.forEach(person => {
        ...
        morningTd.textContent = person.stats.morning;
        row.appendChild(morningTd);
        
        if (schedulingRules.shiftTypes === 3) {
            const midTd = document.createElement('td');
            midTd.textContent = person.stats.mid || 0;
            row.appendChild(midTd);
        }
        
        const eveningTd = document.createElement('td');
        eveningTd.textContent = person.stats.evening;
        row.appendChild(eveningTd);
        
        const restTd = document.createElement('td');
        restTd.textContent = person.stats.rest;
